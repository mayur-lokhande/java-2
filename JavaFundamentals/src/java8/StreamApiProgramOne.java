...
public class StreamApiProgramOne {

	public static void main(String[] args) {
		List<Employee> list = new ArrayList<Employee>();
		list.add(new Employee(1, "Mayur", "Lokhande", 26, "Software Engineer", 50000, "Pune"));
		...
		List<Employee> result1 = list.stream().filter((emp)->emp.getAge()>30 && emp.getLocation()=="Pune").collect(Collectors.toList());
		System.out.println(result1);
		
		//stream to print character sequence matching elements
		List<Employee> result2 = list.stream().filter((emp)->emp.getFirstName().contains("sh")).collect(Collectors.toList());
